...
DIST_DIR=out/$(basename "$KERNEL_DIR")

echo "[*] Kernel dir: $KERNEL_DIR"
echo "[*] DIST_DIR: $DIST_DIR"

echo "[*] Running initial Kleaf build..."
tools/bazel run //common:kernel_aarch64_dist -- --destdir=$DIST_DIR

CONFIG_FILE=$(find bazel-out -name .config | head -n1)

if [ -z "$CONFIG_FILE" ]; then
    echo "[!] .config not found in bazel-out"
    exit 1
fi
...
# finalize config
make O=$(dirname "$CONFIG_FILE") olddefconfig

# Step 2: rebuild with updated config
echo "[*] Rebuilding with Kleaf..."
tools/bazel run //common:kernel_aarch64_dist -- --destdir=$DIST_DIR
...
echo ""
echo "=== Output ==="
echo "Kernel : $DIST_DIR/Image"
echo "Modules: $DIST_DIR/"